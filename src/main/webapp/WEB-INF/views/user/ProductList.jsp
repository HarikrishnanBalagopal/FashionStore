								...
								<li><a href="javascript:;" data-toggle="collapse"
									...
									<ul id="men" class="collapse collapseItem">
										<c:forEach items="${maleCategoryList}" var="maleCategory">
											<li><a
												href="/FashionStore/ProductList?category=${maleCategory.id}"><i
								...
								<li><a href="javascript:;" data-toggle="collapse"
									...
									<ul id="women" class="collapse collapseItem">
										<c:forEach items="${femaleCategoryList}" var="femaleCategory">
											<li><a
												href="/FashionStore/ProductList?category=${femaleCategory.id}"><i
			...
			<div class="col-md-9 col-sm-8 col-xs-12">
				...
				<div class="row productListSingle">
					<div data-ng-repeat="p in products | searchFilter:searchText">
						<div data-product-list-element data-product="p" data-change="quickView(id)"></div>
					</div>
				</div>
			</div>
		</div>
...
app.directive('productListElement',
		['$window', function ($window){
	return {
			restrict: 'A',
			scope:{product: '=', change: '&'},
			replace: true,
			templateUrl: 'resources/templates/ProductListTemplate.html',
			link: function(scope)
			{
				scope.addToCart = function()
				{
					$window.location.href="/FashionStore/AddToCart?id=" + scope.product.id;
				};
			}
		   };
		   }]);
